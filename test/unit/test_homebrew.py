...
def test_update_python_resources_error(mock_subprocess, mock_which):
    ...
    mock_subprocess.assert_called_once_with(
        f"brew developer on && cd {FORMULA_PATH} && {brew_path} update-python-resources {formula_name}",
...
def test_setup_homebrew_tap_error(mock_subprocess, mock_which):
    with pytest.raises(SystemExit):
        setup_homebrew_tap("owner", "tap")

    brew_path = shutil.which("brew")
    mock_subprocess.assert_called_once_with(
        f"{brew_path} tap owner/tap",
        stderr=subprocess.STDOUT,
        text=True,
        timeout=300,
        shell=True,
...
def test_get_homebrew_version_error(mock_subprocess, mock_which):
    with pytest.raises(SystemExit):
        get_homebrew_version()
